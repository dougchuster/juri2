...
interface AuroraBackgroundProps extends HTMLAttributes<HTMLDivElement> {
  ...
  /** Exibe o shader de linhas animadas (para teste). */
  showShader?: boolean;
}
...
export function AuroraBackground({
  ...
  return (
    <div className={cn("relative isolate overflow-hidden", className)} {...props}>
      {/* Camada Shader (linhas animadas — para teste) */}
      {showShader && (
        <div className="pointer-events-none absolute inset-0 z-0 opacity-40 mix-blend-screen" aria-hidden>
          <ShaderAnimation />
        </div>
      )}

      {/* Camada Aurora (original) */}
